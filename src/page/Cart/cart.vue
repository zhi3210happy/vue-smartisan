        ...
        .ui-cart
          div
            ...
            .cart-table-title
              ...
              span.price1 单价
            // 列表
            .cart-table(v-for='(item,i) in cartList', :key='i')
              .cart-group.divide.pr(:data-productid='item.productId')
                .cart-top-items
                  .cart-items.clearfix
                    // 勾选
                    .items-choose
                      span.blue-checkbox-new(:class="{'checkbox-on':item.checked === '1'}", @click="editCart('check',item)")
                    // 图片
                    .items-thumb.fl
                    ...
                    // 删除按钮
                    .operation
                      a.items-delete-btn(@click='cartdel(item.productId)')
                    // 商品数量
                    div
                      // 总价格
                      ...
                      display: flex;\
                      align-items: center;\
                      justify-content: center;', :limit='5', @edit-num='EditNum')
                      // 价格
                      .price1 ¥ {{item.productPrice}}
        .cart-bottom-bg.fix-bottom
          .fix-bottom-inner
            .cart-bar-operation
              div
                .choose-all
                  span.blue-checkbox-new(:class="{'checkbox-on':checkAllFlag}", @click='editCheckAll')
                  | 全选
                .delete-choose-goods
                  | 删除选中的商品
            .shipping
              .shipping-box
                ...
                .shipping-total.shipping-price
                  h4.highlight
                    ...
                    span ￥
                    i(v-text='checkPrice')
                  h5.shipping-tips.ng-scope 应付总额不含运费
              y-button.big-main-btn(:classStyle="checkNum > 0?'main-btn':'disabled-btn'", style='margin: 0;width: 130px;height: 50px;line-height: 50px;font-size: 16px', text='现在结算', @btnClick='checkout')
      div(v-else='', style='padding:50px')
        .cart-e
        p(style='text-align: center;padding: 20px;color: #8d8d8d') 你的购物车空空如也
        div(style='text-align: center')
          router-link(to='/goods')
            y-button(text='现在选购', style='width: 150px;height: 40px;line-height: 38px;color: #8d8d8d')
  y-footer
</template>
<script>

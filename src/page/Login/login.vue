...
.login.v2
  .wrapper
    .dialog.dialog-shadow(style='display: block; margin-top: -362px;')
      .title(v-if='loginPage')
        h4 使用 Smartisan ID 登录官网
      .content(v-if='loginPage')
        ul.common-form
          li.username.border-1p
            .input
          ...
          li.pr(style='text-align: right')
            span.pa(style='top: 0;left: 0;color: #d44d44') {{ruleForm.errMsg}}
            a(href='javascript:;', style='padding: 0 5px', @click='loginPage=false') 注册 Smartisan ID
        // 登陆
        div
          y-button(text='登陆', :classStyle="ruleForm.userPwd&& ruleForm.userName?'main-btn':'disabled-btn'", @btnClick='login', style='margin: 0;width: 100%;height: 48px;font-size: 18px;line-height: 48px')
      .registered(v-else='')
        h4 注册 Smartisan ID
        .content(style='margin-top: 20px;')
          ...
          div
            y-button(:classStyle="registered.userPwd&&registered.userPwd2&&registered.userName?'main-btn':'disabled-btn'", text='注册', style='margin: 0;width: 100%;height: 48px;font-size: 18px;line-height: 48px', @btnClick='regist')
          ul.common-form.pr
            li.pa(style='left: 0;top: 0;margin: 0;color: #d44d44') {{registered.errMsg}}
            li(style='text-align: center;line-height: 48px;margin-bottom: 0;')
              span 如果您已拥有 Smartisan ID，则可在此
              a(href='javascript:;', style='margin: 0 5px', @click='loginPage=true') 登陆
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      login () {
        ...
        const params = {userName: this.ruleForm.userName, userPwd: this.ruleForm.userPwd}
